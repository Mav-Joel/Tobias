...
for x in Command:
    for lettre in x :
        secondaryContent.append(lettre)
...
for x in Command:
    for lettre in x :
        globalPrimairyItems.append(lettre)
...
class creatorWindow(object):
    ...
    def execute(self):
        ...
        for i in range(0,len(featureContent)-1):
            items = self.listWidget_2.item(i).text()
    ...
    def getItems(self):
        ...
        if nObjets > 0:
            ...
            for item in indexObjets :
                ...
                Command = mydb.cursor() 
                Command.execute("USE tobiasdb") 
                Command.execute("SELECT command FROM creator WHERE name='{}'".format(item))

                for x in Command:
                    for lettre in x :                            
                        Commande.append(lettre)
                    
                result = Recup("creator","{}".format(item),"command","name")
    ...
    def setupUi(self, creatorWindow):
        ...
        for i in range(0,len(primaryContent)):
            self.listWidget.insertItem(i,primaryContent[i])
      
        for y in range(0,len(secondaryContent)):
            self.listWidget_3.insertItem(y,secondaryContent[y])

        for z in range(0,len(featureContent)):
            self.listWidget_4.insertItem(z,featureContent[z])
