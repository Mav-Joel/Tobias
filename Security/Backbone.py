...
class Backbone():
    ...
    def innerPortScan(self):
        ...
        if len(self.portList) == len(dbListe): 
            ...
            if answer == "drop":
                ...
                os.system("/Users/{}/Archetype/sysCommands/iptableHandler.bash Backbone {} ".format(Utilisateur,strOpenPorts))
    ...
    def networkSpace(self):
        ...
        if self.monIp != previousIp[0] :
            ...
            Tools().Rapport("Changement Espace Réseau")
            Tools().Rapport(f"Previous IP {previousIp[0]}")
            Tools().Rapport(f"Current IP {self.monIp}")
    ...
    def Liste_Autorisée(self):
        ...
        Liste_IP = str(Liste_IP)
        Liste_IP = Liste_IP.strip("[]")
        Liste_IP = Liste_IP.replace("',","")
        Liste_IP = Liste_IP.replace("'","")
