...
from Bibliotheque.Output_Friendly import *
...
def Scan_Reseau(): #Scan du Réseau actuel , donnant l'adresse Mac et le SSID de tout les réseaux vus

    os.system("wifi on") #Activation du Wifi ##NOTE(SI LE WIFI N EST PAS ALLUME , UNE ERREUR INTERVIENT)  
    Scan=subprocess.getoutput("iwlist wlp2s0 scan | egrep \"ESSID:\" |sort -n|uniq")#Récupération uniquement du SSID
    Mac=subprocess.getoutput("iwlist wlp2s0 scan | egrep \"Address:\"| sed -re \"s/Cell//g\" | sed -re \"s/.. -//g\"|sort -n|uniq")#Récupération uniquement des adresses MAC
    Notification("Wifi activé")
    Label_Text_1 = Label(frame2,text=Scan+Mac,font=("Courrier",10),bg='#16638a', fg="Black") #Fenetre , NAture , Place holder , contenu , police , font , couleur de police
    Label_Text_1.pack(pady=20)
...
class pageReseauUI(object):
        ...
        def Hebergeur(self):
                ...
                while True :
                    ...
                    #Jean
                        print(data)
                        with open("/home/joel/jordanBruno.csv","a") as variable : 
                                variable.write(data)
        ...
        def globalScan(self) :
                ...
                Notification("Network IP Scan ongoing")
                print("Network IP Scan ongoing")
        
                ip = Recup("reseau","currentIp","Information","Network")
