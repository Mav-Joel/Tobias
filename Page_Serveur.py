...
Command.execute("SELECT DISTINCT(nom) FROM knownIps")
...
Command = mydb.cursor() 
Command.execute("USE tobiasdb") 
Command.execute("SELECT DISTINCT(adresse) FROM knownIps")
...
class ServerWindow(object):
    ...
    def send(self):  
        ...
        if self.lineEdit_2.text() != "" :
            ...
            Command = mydb.cursor() 
            Command.execute("USE tobiasdb")
            Command.execute("INSERT INTO servCommands (id, Programme, Information) VALUES (NULL , 'PageServeur', '{}')".format(commande))
            mydb.commit()
